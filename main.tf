...
# EFS backup to S3
module "efs_backup" {
  source                             = "git::https://github.com/cloudposse/terraform-aws-efs-backup.git?ref=tags/0.8.0"
  name                               = "${var.name}"
  stage                              = "${var.stage}"
  namespace                          = "${var.namespace}"
...
module "cicd" {
  source              = "git::https://github.com/osulli/terraform-aws-cicd.git?ref=osulli:patch-1"
  namespace           = "${var.namespace}"
  name                = "${var.name}"
  stage               = "${var.stage}"
  app                 = "${module.elastic_beanstalk_application.app_name}"
